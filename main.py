...
def TrackImages():
    ...
    attendance_list = []  # 🔧 Store all records
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if conf < 60:  # 🔧 slightly relaxed threshold
                ...
                id_val = df.loc[df['SERIAL NO.'] == serial]['ID'].values[0]

                cv2.putText(im, f"{name}", (x, y + h), font, 1, (255, 255, 255), 2)
                attendance_list.append([id_val, name, date, timeStamp])  # 🔧 collect each
    ...
    if attendance_list:
        ...
        df_att = pd.DataFrame(attendance_list, columns=col_names)
        df_att.to_csv(file_path, index=False)
        mess.showinfo("Success", f"Attendance saved successfully at {file_path}")
        # ✅ Refresh the Attendance Table in the UI
        try:
            for k in tv.get_children():
                tv.delete(k)
            ...
            if os.path.exists(file_path):
                with open(file_path, 'r') as csvFile:
                    ...
                    for row in reader:
                        if len(row) >= 4:
                            tv.insert('', 'end', text=row[0], values=(row[1], row[2], row[3]))
                csvFile.close()
            else:
                mess.showinfo("Info", "Attendance file not found.")
        except Exception as e:
            mess.showinfo("Error", f"Unable to refresh attendance table: {e}")

    else:
        mess.showinfo("Info", "No recognized faces recorded.")
...
def send_email():
    ...
    from_email = "ch.sai1717@gmail.com"  # Update with your email
    password = "phlz pduk glls hfuj"  # Update with your email password
